...
class TrueTypeFont extends BaseFont {
     ...
     * of the table.
     */
    protected LinkedHashMap tables;
    /** The file in use.
     */
     ...
     * units.
     */
    protected LinkedHashMap cmap10;
     ...
     * units.
     */
    protected LinkedHashMap cmap31;

    protected LinkedHashMap cmapExt;
    ...
    LinkedHashMap readFormat12() throws IOException {
     ...
     * @return a <CODE>LinkedHashMap</CODE> representing this map
     * @throws IOException the font file could not be read
     */
    LinkedHashMap readFormat0() throws IOException {
        LinkedHashMap h = new LinkedHashMap();
        rf.skipBytes(4);
        for (int k = 0; k < 256; ++k) {
            int r[] = new int[2];
     ...
     * @throws IOException the font file could not be read
     */
    LinkedHashMap readFormat4() throws IOException {
        LinkedHashMap h = new LinkedHashMap();
     ...
     * @throws IOException the font file could not be read
     */
    LinkedHashMap readFormat6() throws IOException {
        LinkedHashMap h = new LinkedHashMap();
        rf.skipBytes(4);
    ...
    protected void addRangeUni(LinkedHashMap longTag, boolean includeMetrics, boolean subsetp) {
    ...
    void writeFont(PdfWriter writer, PdfIndirectReference ref, Object params[]) throws DocumentException, IOException {
        ...
        if (embedded) {
            if (cff) {
                ...
                if (subsetp)
                    subsetPrefix = createSubsetPrefix(writer.getSignDate().getTimeInMillis());
                LinkedHashMap glyphs = new LinkedHashMap();
                for (int k = firstChar; k <= lastChar; ++k) {
                    if (shortTag[k] != 0) {
                        int[] metrics = null;
                        if (specialMap != null) {
