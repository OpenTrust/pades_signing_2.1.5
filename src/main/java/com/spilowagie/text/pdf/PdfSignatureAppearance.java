...
import com.spilowagie.text.Phrase;
...
public class PdfSignatureAppearance {
    ...
    private LinkedHashMap exclusionLocations;
        
    PdfSignatureAppearance(PdfStamperImp writer) {
        this.writer = writer;
        signDate = new GregorianCalendar();
        fieldName = getNewSigName();
        writer.setSignDate(signDate);
    }
    ...
    */    
    ...
    public void setVisibleSignature(Rectangle pageRect, int page, String fieldName) {
        if (fieldName != null) {
            if (fieldName.indexOf('.') >= 0)
                throw new IllegalArgumentException("PDF signature field names cannot contain a dot."); // Patch OT
            AcroFields af = writer.getAcroFields();
            AcroFields.Item item = af.getFieldItem(fieldName);
            if (item != null)
                throw new IllegalArgumentException("The field " + fieldName + " already exists in the PDF file."); // OT Patch : modified text
     ...
     */    
    public com.spilowagie.text.pdf.PdfName getFilter() {
        return filter;
    }
    ...
    /**
     * Sets the field name. // Patch OT
     */    
    public  void setFieldName(java.lang.String newName) {
     ...
     */    
    public com.spilowagie.text.Rectangle getPageRect() {
        return pageRect;
    }
    ...
    public void setSignDate(java.util.Calendar signDate) {
    	writer.setSignDate(signDate); // Addition Patch OT
        this.signDate = signDate;
    }
    
    com.spilowagie.text.pdf.ByteBuffer getSigout() {
        return sigout;
    }
    
    void setSigout(com.spilowagie.text.pdf.ByteBuffer sigout) {
        this.sigout = sigout;
    }
     ...
     * byte_size*2+2.
     * @param exclusionSizes a <CODE>LinkedHashMap</CODE> with names and sizes to be excluded in the signature
     ...
     * @throws DocumentException on error
     */    
    public void preClose(LinkedHashMap exclusionSizes) throws IOException, DocumentException {
        if (preClosed)
        ...
        //FIXME : always or only for PAdES v3 ?
        //FIXME : error if could not add to catalog because baseversion>version ? Adobe Pro X produces documents with for instance PdfVersion=1.4 and extension.BaseVersion=1.7
        writer.getPdfVersion().addToCatalog(writer.reader.getCatalog());
        ...
        log.debug(Channel.TECH, "preClosing pdf, byteRangePosition is written at byte %1$s", byteRangePosition);
        ...
        if (tempFile == null) {
            ...
            bf.append(']');
            log.debug(Channel.TECH, "preClosing pdf with a null tempFile, byterange written in doc is %1$s", bf);
            ...
            try {
                ...
                for (int k = 0; k < range.length; ++k)
                    bf.append(range[k]).append(' ');
                bf.append(']');
                log.debug(Channel.TECH, "preClosing pdf with a non null tempFile, byterange written in doc is %1$s", bf);
                raf.seek(byteRangePosition);
                raf.write(bf.getBuffer(), 0, bf.size());
            }
            catch (IOException e) {
            	closeStreams(); // OT Patch
                throw e;
            }
        }
    }
     ...
     * same keys as the ones provided in {@link #preClose(LinkedHashMap)}.
     * @param update a <CODE>PdfDictionary</CODE> with the key/value that will fill the holes defined
     * in {@link #preClose(LinkedHashMap)}
     * @throws DocumentException on error
     * @throws IOException on error
     */    
    public void close(PdfDictionary update) throws IOException, DocumentException {
        try {
            ...
            for (Iterator it = update.getKeys().iterator(); it.hasNext();) {
                ...
                if (tempFile == null) {
                    log.debug(Channel.TECH, "Arraycopying signature with size %1$s at position %2$s", bf.size(), lit.getPosition());
                    System.arraycopy(bf.getBuffer(), 0, bout, lit.getPosition(), bf.size());
                } else {
                    log.debug(Channel.TECH, "Raf-writing signature with size %1$s at position %2$s", bf.size(), lit.getPosition());
            ...
            if (tempFile == null) {
                log.debug(Channel.TECH, "Copying pdf content to originalout using os.write(whole buffer)");
                originalout.write(bout, 0, boutLen);
            }
            else {
                if (originalout != null) {
                    log.debug(Channel.TECH, "Copying pdf content to originalout using os.write(chunk by chunk)");
                    raf.seek(0);
                ...
                } else log.debug(Channel.TECH, "originalout is null");
            }
        }
        finally {
            if (tempFile != null) {
     ...
     */    
    public com.spilowagie.text.pdf.PdfDictionary getCryptoDictionary() {
     ...
     */    
    public void setCryptoDictionary(com.spilowagie.text.pdf.PdfDictionary cryptoDictionary) {
     ...
     */    
    public com.spilowagie.text.pdf.PdfStamper getStamper() {
        return stamper;
    }
    
    void setStamper(com.spilowagie.text.pdf.PdfStamper stamper) {
     ...
     */    
    public com.spilowagie.text.pdf.PdfSigGenericPKCS getSigStandard() {
        return sigStandard;
    }
