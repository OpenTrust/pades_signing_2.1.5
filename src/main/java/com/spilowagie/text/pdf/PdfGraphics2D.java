...
import com.spilowagie.text.pdf.internal.PolylineShape;
...
public class PdfGraphics2D extends Graphics2D {
    ...
    private LinkedHashMap baseFonts;
    ...
    PdfGraphics2D(PdfContentByte cb, float width, float height, FontMapper fontMapper, boolean onlyShapes, boolean convertImagesToJPEG, float quality) {
        ...
        this.baseFonts = new LinkedHashMap();
        if (!onlyShapes) {
...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            com.spilowagie.text.Image image = null;
            if(!convertImagesToJPEG){
                image = com.spilowagie.text.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = com.spilowagie.text.Image.getInstance(baos.toByteArray());
                
            }
            if (mask!=null) {
                com.spilowagie.text.Image msk = com.spilowagie.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                com.spilowagie.text.Image image = com.spilowagie.text.Image.getInstance(img, null);
