...
import com.spilowagie.text.xml.xmp.XmpReader;

class PdfStamperImp extends PdfWriter {
    LinkedHashMap readers2intrefs = new LinkedHashMap();
    LinkedHashMap readers2file = new LinkedHashMap();
    RandomAccessFileOrArray file;
    PdfReader reader;
    IntHashtable myXref = new IntHashtable();
    /** Integer(page number) -> PageStamp */
    LinkedHashMap pagesToContent = new LinkedHashMap();
    ...
    protected LinkedHashMap fieldTemplates = new LinkedHashMap();
    ...
    void close(LinkedHashMap moreInfo) throws IOException {
        if (closed)
            return;
        if (useVp) {
        ...
        if (oldInfo != null && oldInfo.get(PdfName.PRODUCER) != null)
        	producer = oldInfo.getAsString(PdfName.PRODUCER).toUnicodeString(); // OT Patch
        if (producer == null) {
        ...
        // Patch OT
        PdfDate date = null;
        if(signDate!=null) date = new PdfDate(signDate);
        else date = new PdfDate();
        // End Patch OT
        ...
        if (crypto != null) {
         ...
         // TODO : deal with fileID generation for encrypted files as well as non-encryptes files
        }
        else
        	fileID = createFileId(); // Modification Patch OT
	...
	// Addition Patch OT. Inspired from PdfEncryption.createInfoId and PdfEncryption.createDocumentId
    // TODO : add another alea (only signDate for now)
    public PdfObject createFileId() {
		MessageDigest md5;
		try {
		...
		if(signDate==null) signDate = new GregorianCalendar();
		byte[] id = md5.digest((""+signDate.getTimeInMillis()).getBytes());
		ByteBuffer buf = new ByteBuffer(90);
		buf.append('[').append('<');
		for (int k = 0; k < 16; ++k)
			buf.appendHex(id[k]);
		buf.append('>').append('<');
		id = md5.digest((""+signDate.getTimeInMillis()+1).getBytes());
		for (int k = 0; k < 16; ++k)
			buf.appendHex(id[k]);
		buf.append('>').append(']');
    ...
    public void addComments(FdfReader fdf) throws IOException{
        ...
        LinkedHashMap irt = new LinkedHashMap();
    ...
    void correctAcroFieldPages(int page) {
        ...
        if (page > reader.getNumberOfPages())
            return;
        LinkedHashMap fields = acroFields.getFields();
...
//        reader.getCatalog().remove(PdfName.ACROFORM);
    ...
    /**
     * @see com.spilowagie.text.pdf.PdfWriter#getPageReference(int)
     */
    ...
    /**
     * @see com.spilowagie.text.pdf.PdfWriter#addAnnotation(com.spilowagie.text.pdf.PdfAnnotation)
     */
    ...
    void addAnnotation(PdfAnnotation annot, PdfDictionary pageN) {
        try {
            ...
            for (int k = 0; k < allAnnots.size(); ++k) {
                ...
                if (annot.isForm()) {
                    if (!annot.isUsed()) {
                        LinkedHashMap templates = annot.getTemplates();
                        if (templates != null)
    ...
    void setJavaScript() throws IOException {
        LinkedHashMap djs = pdf.getDocumentLevelJS();
        if (djs.isEmpty())
            return;
        PdfDictionary catalog = reader.getCatalog();
    ...
    void addFileAttachments() throws IOException {
        LinkedHashMap fs = pdf.getDocumentFileAttachment();
        if (fs.isEmpty())
            return;
        PdfDictionary catalog = reader.getCatalog();
        PdfDictionary names = (PdfDictionary)PdfReader.getPdfObject(catalog.get(PdfName.NAMES), catalog);
        ...
        markUsed(names);
        LinkedHashMap old = PdfNameTree.readTree((PdfDictionary)PdfReader.getPdfObjectRelease(names.get(PdfName.EMBEDDEDFILES)));
    ...
    /**
     * @see com.spilowagie.text.pdf.PdfWriter#setOpenAction(com.spilowagie.text.pdf.PdfAction)
     */
    ...
    /**
     * @see com.spilowagie.text.pdf.PdfWriter#setOpenAction(java.lang.String)
     */
    ...
    /**
     * @see com.spilowagie.text.pdf.PdfWriter#setThumbnail(com.spilowagie.text.Image)
     */
    public void setThumbnail(com.spilowagie.text.Image image) {
    ...
    protected void readOCProperties() {
    	...
    	PdfLayer layer;
    	LinkedHashMap ocgmap = new LinkedHashMap();
     ...
     * @param	ocgmap	a LinkedHashMap with indirect reference Strings as keys and PdfLayer objects as values.
     * @since	2.1.2
     */
