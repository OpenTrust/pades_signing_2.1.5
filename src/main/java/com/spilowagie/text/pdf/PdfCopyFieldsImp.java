...
class PdfCopyFieldsImp extends PdfWriter {
    ...
    LinkedHashMap pages2intrefs = new LinkedHashMap();
    LinkedHashMap visited = new LinkedHashMap();
    ArrayList fields = new ArrayList();
    RandomAccessFileOrArray file;
    LinkedHashMap fieldTree = new LinkedHashMap();
    ...
    Document nd;
    private LinkedHashMap tabOrder;
...
//            addToBody(obj, refo);
    ...
    protected PdfArray branchForm(LinkedHashMap level, PdfIndirectReference parent, String fname) throws IOException {
        PdfArray arr = new PdfArray();
        for (Iterator it = level.entrySet().iterator(); it.hasNext();) {
            ...
            if (obj instanceof LinkedHashMap) {
                dic.put(PdfName.KIDS, branchForm((LinkedHashMap)obj, ind, fname2));
                arr.add(ind);
                addToBody(dic, ind);
            }
            else {
    ...
    void addPageOffsetToField(LinkedHashMap fd, int pageOffset) {
        if (pageOffset == 0)
            return;
    ...
    void mergeWithMaster(LinkedHashMap fd) {
    ...
    void mergeFields() {
        ...
        for (int k = 0; k < fields.size(); ++k) {
            LinkedHashMap fd = ((AcroFields)fields.get(k)).getFields();
    ...
    protected static final LinkedHashMap widgetKeys = new LinkedHashMap();
    protected static final LinkedHashMap fieldKeys = new LinkedHashMap();
    static {
