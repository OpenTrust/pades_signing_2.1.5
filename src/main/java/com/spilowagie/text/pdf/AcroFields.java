...
public class AcroFields {
	private static SPILogger log = SPILogger.getLogger("PDFSIGN"); // Ajout OT

    PdfReader reader;
    PdfWriter writer;
    LinkedHashMap fields;
    private int topFirst;
    private LinkedHashMap sigNames;
    private boolean append;
    public static final int DA_FONT = 0;
    public static final int DA_SIZE = 1;
    public static final int DA_COLOR = 2;
    private LinkedHashMap extensionFonts = new LinkedHashMap();
    private XfaForm xfa;
    ...
    private LinkedHashMap localFonts = new LinkedHashMap();
     ...
     * @throws com.spilowagie.text.DocumentException o error
     */
    ...
    public void setFields(FdfReader fdf) throws IOException, DocumentException {
        LinkedHashMap fd = fdf.getFields();
    ...
    public void setFields(XfdfReader xfdf) throws IOException, DocumentException {
        LinkedHashMap fd = xfdf.getFields();
        for (Iterator i = fd.keySet().iterator(); i.hasNext();) {
            String f = (String)i.next();
    ...
    Map sigNamesWithoutDocTS;

    /**
     * Gets the field names that have signatures and are signed.
     * if withDocTS, also gets the DocumentTimeStamp entries in the signature dictionary
     * 
     * @return the field names that have signatures and are signed
     */
    public ArrayList getSignatureNames(boolean withDocTS) {
        if (withDocTS && sigNames != null)
            return new ArrayList(sigNames.keySet());
        else if (!withDocTS && sigNamesWithoutDocTS != null)
            return new ArrayList(sigNamesWithoutDocTS.keySet());
        sigNames = new LinkedHashMap();
        sigNamesWithoutDocTS = new LinkedHashMap();
        ...
        for (Iterator it = fields.entrySet().iterator(); it.hasNext();) {
            ...
            if (rangeSize < 2)
                continue;
            int docTSFlag = 0;
            if(PdfName.ETSI_RFC3161.equals(v.getAsName(PdfName.SUBFILTER)))
            	docTSFlag = 1;
            int length = ro.getAsNumber(rangeSize - 1).intValue() + ro.getAsNumber(rangeSize - 2).intValue();
            sorter.add(new Object[]{entry.getKey(), new int[]{length, docTSFlag}});
        }
        ...
        if (!sorter.isEmpty()) {
            ...
            for (int k = 0; k < sorter.size(); ++k) {
                ...
                p[1] = k + 1;
                sigNames.put(name, p);
                if(docTSFlag==0) sigNamesWithoutDocTS.put(name, p);
            }
        }
        if (withDocTS)
        	return new ArrayList(sigNames.keySet());
    ...
    public PdfPKCS7 verifySignature(String name, String provider) {
        log.debug(Channel.TECH, "Starting verification for signature %1$s", name);
        PdfDictionary v = getSignatureDictionary(name);
        if (v == null) {
        	log.error(Channel.TECH, "No dictionary found for signature %1$s", name);
            return null;
        }
        try {
            ...
            if (sub.equals(PdfName.ADBE_X509_RSA_SHA1)) {
                pk = new PdfPKCS7(contents.getOriginalBytes(), cert.getBytes(), provider, this, name);
            } else {
                pk = new PdfPKCS7(contents.getOriginalBytes(), provider, sub, this, name);
                pk.setDictionaryCert(cert==null?null:cert.getBytes());
            }
            pk.setXRefComputedRevision(reader.reverseRevisionNbr - v.getReverseRevision());
            updateByteRange(pk, v);
            PdfString str = v.getAsString(PdfName.M);
            if (str != null) {
            	Calendar signingTime = PdfDate.decode(str.toString());
            	log.debug(Channel.TECH, "Signing time '%1$s' found in signature dictionary", signingTime);
                pk.setDicoSignDate(signingTime);
            }
            PdfObject obj = PdfReader.getPdfObject(v.get(PdfName.NAME));
            if (obj != null) {
            	String signName = null;
            	if (obj.isString())
            		signName = ((PdfString)obj).toUnicodeString();
            	else if(obj.isName())
            		signName = PdfName.decodeName(obj.toString());
            	log.debug(Channel.TECH, "Name '%1$s' found in signature dictionary", signName);
            	pk.setSignName(signName);
            }
            str = v.getAsString(PdfName.REASON);
            if (str != null) {
            	String reason = str.toUnicodeString();
            	log.debug(Channel.TECH, "Reason '%1$s' found in signature dictionary", reason);
                pk.setReason(reason);
            }
            str = v.getAsString(PdfName.LOCATION);
            if (str != null) {
            	String location = str.toUnicodeString();
            	log.debug(Channel.TECH, "Location '%1$s' found in signature dictionary", location);
                pk.setLocation(location);
            }
            str = v.getAsString(PdfName.CONTACTINFO);
            if (str != null) {
            	String contactinfo = str.toUnicodeString();
            	log.debug(Channel.TECH, "ContactInfo '%1$s' found in signature dictionary", contactinfo);
            	pk.setContactInfo(contactinfo);
            }
            PdfArray b = v.getAsArray(PdfName.BYTERANGE);
            int firstChunkStart = b.getAsNumber(0).intValue();
            int signaturePosition = b.getAsNumber(1).intValue();
			int signaturePositionEnd = b.getAsNumber(2).intValue();
			int lastChunkSize = b.getAsNumber(3).intValue();
            int[] byteRange = new int[] {firstChunkStart, signaturePosition, signaturePositionEnd,lastChunkSize}; 
			pk.setByteRange(byteRange);
            
            //TODO : also read Prop_Build dictionary, and pass the info to pk
            return pk;
        }
        catch (Exception e) {
            throw new ExceptionConverter(e);
        ...
        try {
            ...
            for (int k = 0; k < b.size(); ++k) {
                ...
                while (length > 0) {
                    ...
                    if (rd <= 0)
                        throw new Exception("Invalid byterange, goes beyond the length of the document");
                    length -= rd;
    ...
    public int getRevisionLength(String field) {
        getSignatureNames();
        field = getTranslatedFieldName(field);
        if (!sigNames.containsKey(field))
            return 0;
     ...
     * @since	2.1.5	this method used to return a LinkedHashMap
     */
     ...
     * LinkedHashMap cache = new LinkedHashMap();// the appearances cache
     ...
     * @since	2.1.5	this method used to take a LinkedHashMap as parameter
     */
    ...
    private static final LinkedHashMap stdFieldFontNames = new LinkedHashMap();
     ...
     * 
     * @since	2.1.5	this used to be a LinkedHashMap
     */
