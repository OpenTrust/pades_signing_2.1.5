...
public class CFFFontSubset extends CFFFont {
	...
	*/
	...
	/**
	 * A LinkedHashMap containing the glyphs used in the text after being converted
	 * to glyph number by the CMap 
	 */
	LinkedHashMap GlyphsUsed;
	/**
	 * The GlyphsUsed keys as an ArrayList
	 */
	ArrayList glyphsInList;
	/**
	 * A LinkedHashMap for keeping the FDArrays being used by the font
	 */
	LinkedHashMap FDArrayUsed = new LinkedHashMap();
	/**
	 * A HashMaps array for keeping the subroutines used in each FontDict
	 */
	LinkedHashMap[] hSubrsUsed;
	/**
	 * The SubroutinesUsed HashMaps as ArrayLists
	 */
	ArrayList[] lSubrsUsed;
	/**
	 * A LinkedHashMap for keeping the Global subroutines used in the font
	 */
	LinkedHashMap hGSubrsUsed  = new LinkedHashMap();
	/**
	 * The Global SubroutinesUsed HashMaps as ArrayLists
	 */
	ArrayList lGSubrsUsed = new ArrayList();
	/**
	 * A LinkedHashMap for keeping the subroutines used in a non-cid font
	 */
	LinkedHashMap hSubrsUsedNonCID  = new LinkedHashMap();
	/**
	 * The SubroutinesUsed LinkedHashMap as ArrayList
	 */
	 ...
	 * @param GlyphsUsed - a LinkedHashMap that contains the glyph used in the subset 
	 */
    public CFFFontSubset(RandomAccessFileOrArray rf,LinkedHashMap GlyphsUsed){
	...
	/**
	 * Function reads the FDSelect and builds the FDArrayUsed LinkedHashMap According to the glyphs used
	...
	protected void BuildFDArrayUsed(int Font)
	{
		...
		for (int i=0;i<glyphsInList.size();i++)
		{
			...
			// Put the FD index into the FDArrayUsed LinkedHashMap
	...
	protected void BuildNewLGSubrs(int Font)throws IOException
	{
		...
		if(fonts[Font].isCID)
		{
			...
			hSubrsUsed = new LinkedHashMap[fonts[Font].fdprivateOffsets.length];
			...
			for (int j=0;j<FDInList.size();j++)
			{
				...
				hSubrsUsed[FD] = new LinkedHashMap();
	...
	/**
	 * Function uses ReadAsubr on the glyph used to build the LSubr & Gsubr LinkedHashMap.
	 * The LinkedHashMap (of the lsubr only) is then scanned recursively for Lsubr & Gsubrs
	 * calls.  
	 * @param Font the font
	 ...
	 * @param hSubr LinkedHashMap of the subrs used
	 * @param lSubr ArrayList of the subrs used
	 */
	protected void BuildSubrUsed(int Font,int FD,int SubrOffset,int[] SubrsOffsets,LinkedHashMap hSubr,ArrayList lSubr)
	{
	 ...
	 * @param hSubr the LinkedHashMap for the lSubrs
	 * @param lSubr the ArrayList for the lSubrs
	 */
	protected void ReadASubr(int begin,int end,int GBias,int LBias,LinkedHashMap hSubr,ArrayList lSubr,int[] LSubrsOffsets)
	{
        ...
        while (getPosition() < end)
        {
        	...
        	if (key=="callsubr") 
        	{
        		...
        		if (NumOfArgs > 0)
        		{
            		...
            		// If the subr isn't in the LinkedHashMap -> Put in
	 ...
	 * @throws IOException
	 */
	protected byte[] BuildNewIndex(int[] Offsets,LinkedHashMap Used,byte OperatorForUnusedEntries) throws IOException 
	{
		int unusedCount = 0;
		int Offset=0;
		...
		for (int i=0;i<Offsets.length;++i)
		{
			...
			// LinkedHashMap then increment the offset var by its size
