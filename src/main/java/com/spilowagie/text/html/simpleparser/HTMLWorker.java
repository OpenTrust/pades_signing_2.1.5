...
package com.spilowagie.text.html.simpleparser;
...
import com.spilowagie.text.ElementTags;
import com.spilowagie.text.ExceptionConverter;
import com.spilowagie.text.FontFactoryImp;
...
import com.spilowagie.text.TextElementArray;
import com.spilowagie.text.html.HtmlTags;
import com.spilowagie.text.html.Markup;
import com.spilowagie.text.pdf.PdfPTable;
import com.spilowagie.text.pdf.draw.LineSeparator;
import com.spilowagie.text.xml.simpleparser.SimpleXMLDocHandler;
import com.spilowagie.text.xml.simpleparser.SimpleXMLParser;
...
public class HTMLWorker implements SimpleXMLDocHandler, DocListener {
	...
	private LinkedHashMap interfaceProps;
	...
	public void setInterfaceProps(LinkedHashMap interfaceProps) {
	...
	public LinkedHashMap getInterfaceProps() {
	...
	public static ArrayList parseToList(Reader reader, StyleSheet style,
			LinkedHashMap interfaceProps) throws IOException {
	...
	public void startElement(String tag, LinkedHashMap h) {
		...
		try {
			...
			if (tag.equals(HtmlTags.IMAGE)) {
				...
				if (interfaceProps != null) {
					...
					if (img == null) {
						LinkedHashMap images = (LinkedHashMap) interfaceProps
								.get("img_static");
						if (images != null) {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				com.spilowagie.text.List list = new com.spilowagie.text.List(false);
				try{
			...
			if (tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				com.spilowagie.text.List list = new com.spilowagie.text.List(true);
				try{
					list.setIndentationLeft(new Float(cprops.getProperty("indent")).floatValue());
				}catch (Exception e) {
					list.setAutoindent(true);
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)
				...
				if (stack.empty())
					return;
				Object obj = stack.pop();
				if (!(obj instanceof com.spilowagie.text.List)) {
					stack.push(obj);
					return;
				}
				if (stack.empty())
			...
			if (tag.equals(HtmlTags.LISTITEM)) {
				...
				if (!(list instanceof com.spilowagie.text.List)) {
					stack.push(list);
					return;
				}
				ListItem item = (ListItem) obj;
				((com.spilowagie.text.List) list).add(item);
	...
	public static final LinkedHashMap tagsSupported = new LinkedHashMap();
